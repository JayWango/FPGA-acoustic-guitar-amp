...
int32_t process_tremolo(int32_t input) {
    ...
    // We want gain to oscillate smoothly between min_gain and max_gain
    // min_gain = 256 - tremolo_depth (when sine is at trough, value = 1)
    // max_gain = 256 (when sine is at peak, value = 255)
    // 
    // The sine table values range from 1 (trough) to 255 (peak), centered at 128
    // We need to map this to gain modulation from 0 to tremolo_depth
    //
    // Map sine value (1-255) to modulation amount (0 to tremolo_depth):
    // When sine = 1 (trough):   modulation = 0,        gain = min_gain = 256 - depth
    // When sine = 128 (zero):   modulation = depth/2,  gain = 256 - depth/2 (midpoint)
    // When sine = 255 (peak):   modulation = depth,    gain = max_gain = 256
    //
    // Formula: modulation = (sine_value - 1) * tremolo_depth / 254
    // This maps the range [1, 255] to [0, tremolo_depth]
    ...
    // GATE: Only apply tremolo to signals above noise floor
    // This prevents modulating DC offset or quantization noise
    int32_t input_abs = (input < 0) ? -input : input;
    const int32_t NOISE_FLOOR = 50;
    
    if (input_abs < NOISE_FLOOR) {
        // Signal too small - bypass tremolo to avoid creating unwanted tones
        return input;
    }
